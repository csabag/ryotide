"""Probe: S / Notes / Q / Q (notes on discarded KV branches; state context stays clean). For states > 500 chars, read the state in ~250-char chunks
...
def take_notes(state):
    ...
    for c in chunks(enc(state)):
        ...
        br = fork_cache(cache)
        row = clf.logits_at_end(br, enc(" (note to self:"))
        ids = []
...
out = open("results/probes/notes_items.jsonl", "w"); R = {"base": [], "notes": []}; nstates = 0

"""Probe: S / Notes / Q / Q, QUESTION-AWARE notes: each branch is prompted with
(notes for question "<q>": ...). Branch discarded, so the question never enters the state context. For states > 500 chars, read the state in ~250-char chunks
...
def take_notes(state, q):
    ...
    for c in chunks(enc(state)):
        ...
        br = fork_cache(cache)
        row = clf.logits_at_end(br, enc(f' (notes for question "{q}":'))
        ids = []
...
out = open("results/probes/notes_q_items.jsonl", "w"); R = {"base": [], "notes": []}; nstates = 0
